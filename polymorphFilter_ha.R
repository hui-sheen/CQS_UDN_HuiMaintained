...
### UPDATE 06/04/2017: change default freqFile from exac to gnomad_exomes.
options(echo=F)
### previous file also includes ANNOVAR annotation columns.
polymorphFilter <- function(previous, th = 0.01, n_Homo = 5,gnomaddata) {
	#### extract udnID ##########
	th <- as.numeric(th)
	...
	gnomAD <- (is.na(gnomAD.eCol) | gnomAD.eCol<=th) & (is.na(gnomAD.gCol) | gnomAD.gCol<=th) # Added 3/20/2017
	...
	step4.2 <- gnomad_freqFilter(paste0(udnID,'_step4.1.txt'),th=th,n_Homo=n_Homo,gnomaddata,step='4.2') 
...
gnomad_freqFilter <- function(previous,th=0.001,n_Homo=5,gnomaddata,step='4.3') {
	th <- as.numeric(th)
...
gnomaddata=args[[4]]
polymorphFilter(args[[1]], th, n_Homo,gnomaddata)
