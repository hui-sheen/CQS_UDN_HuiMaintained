...
SeqPlatform=1    ### 1 Baylor, 2 Hudson Alpha

ProBand_ID=UDN763776
ProBand_Gender=2     ### Male 1, Female 2
...
BroSisIDs=(UDN982934 UDN108104)   ### use space to separate different IDs
BroSisGenders=(1 1)    ### use space to separate different Genders
BroSisAffStat=(1 1)    ### use space to separate different affected status
...
RAW_VCF_DIR=/scratch/cqs/udn/DK_UDN763776/   ### Raw VCF directory, you'd better put all $UDN**.vcf under the same folder 
WORK_DIR=/scratch/cqs/udn/DK_UDN763776/hui3    ### Working directory
PhenoFile=/scratch/cqs/udn/DK_UDN763776/terms.txt
...
tail -n +2 $IH_DIR\/$ProBand_ID\_finalFiltered.txt | cut -f 3 | sort | uniq > prioritization/$ProBand_ID\_candGenes.txt
sed -i '/,/s/,.*//' prioritization/$ProBand_ID\_candGenes.txt
cp $PhenoFile prioritization/$ProBand_ID\_terms.txt
disease_annotation.pl prioritization/$ProBand_ID\_terms.txt -f -p -ph -logistic --gene prioritization/$ProBand_ID\_candGenes.txt -out prioritization/$ProBand_ID 2>prioritization/phenolyzer.log
